...
// The transient corner toast (`ToastView` / `ToastOverlay`) was removed with TASK-704: a floating
// capsule is a web and iOS pattern, and on the Mac that message belongs in the window's status bar.
// `StatusBar` renders `transient` now, including its inline action. Nothing else changed here —
// anything actionable or failed still lands in `notifications` for the bell.
