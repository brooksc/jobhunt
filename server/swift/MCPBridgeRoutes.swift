...
private func handleMCPSitesList(_ request: HTTPRequest, store: BackgroundStore) async -> HTTPResponse {
    do {
...
private func handleMCPSiteUpdate(_ request: HTTPRequest, siteService: SiteService) async -> HTTPResponse {
    ...
    let state: SiteState?
    if let stateStr = req.state {
        state = SiteState(rawValue: stateStr)
    } else {
        state = nil
    }
...
private func handleMCPSnapshot(_ request: HTTPRequest, store: BackgroundStore) async -> HTTPResponse {
    do {
