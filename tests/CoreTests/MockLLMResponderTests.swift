...
final class MockLLMResponderTests: XCTestCase {
    ...
    func testClassify_bySchemaName() {
        XCTAssertEqual(MockLLMResponder.classify(body: ["response_format": ["json_schema": ["name": "fit_score"]]]), .fit)
        XCTAssertEqual(MockLLMResponder.classify(body: ["response_format": ["json_schema": ["name": "job_extraction"]]]), .extraction)
    }
    ...
    func testClassify_fallbackByResumeKeyword() {
        ...
        XCTAssertEqual(MockLLMResponder.classify(body: ["messages": [["role": "user", "content": "Score this resume against the job"]]]), .fit)
        XCTAssertEqual(MockLLMResponder.classify(body: ["messages": [["role": "user", "content": "Extract fields from this posting"]]]), .extraction)
    }

    // MARK: - Input-aware title

    func testPageTitleRecoveredFromPrompt() {
        let data = body(schemaName: "job_extraction",
                        messages: [("user", "URL: https://x\nPage title: Senior iOS Engineer - Acme\nDescription: ...")])
        let parsed = (try? JSONSerialization.jsonObject(with: data)) as? [String: Any]
        XCTAssertEqual(MockLLMResponder.pageTitle(in: parsed), "Senior iOS Engineer - Acme")
    }

    func testTitleAndCompanySplit() {
        XCTAssertEqual(MockLLMResponder.titleAndCompany(from: "Senior iOS Engineer - Acme").title, "Senior iOS Engineer")
    ...
    func testFitContentHasTheFiveRequiredDimensions() throws {
        ...
        XCTAssertEqual(names, ["required_qualifications", "preferred_qualifications", "skills", "experience_level", "domain_fit"])
    }

    func testChatCompletionEnvelopeWrapsStructuredContent() throws {
        let response = MockLLMResponder.chatCompletion(requestBody: body(schemaName: "job_extraction",
            messages: [("user", "Page title: Engineer - Acme\n")]))
