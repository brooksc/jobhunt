...
| `--normalize-seniority [--store <path>]` | Collapse stored `seniority` onto the canonical bands (intern/entry/mid/senior/lead/staff/principal/manager/director/executive). Values carrying no level — "5+ years", "III" — become null rather than a guessed band, because that text feeds the fit-scoring prompt's `experience_level` dimension. Idempotent. | same |
| `--repair-salaries [--store <path>]` | Re-run the current salary normalization over every job's stored pay evidence (its `salaryNote` + its capture's cleaned description) and fix bands the old parser invented. Until the parser required affirmative pay evidence, any two dash-separated numbers over 1,000 became a band — job #1502 (SageSure) states no pay yet stored $2,020–$2,023 from "Best Places to Work in Insurance … (2020-2023)". A stored band is **cleared only when the evidence can't support it** (neither end appears as a money amount), so a figure the model read out of prose the regexes can't re-find survives. Never touches a manually-overridden salary field. Idempotent. | same |
