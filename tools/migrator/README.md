...
| `--repair-remote-types` | Restore work arrangements the old post-extraction clamp erased (TASK-708). Until TASK-705, extraction nulled `Job.remoteType` whenever the user's settings disallowed that arrangement — 223 jobs on the real store (171 hybrid, 52 onsite), which then bucketed as "arrangement not stated", hid from the *Doesn't meet criteria* filter and were flagged defective by the quality checker. The model's answer survived in `extractedJSON`, so this re-reads it — **zero LLM calls**. **Repair, not re-derivation**: never overwrites a job that already states an arrangement, never infers one from location text (that's `--recompute-criteria`), skips a hand-edited `remoteType`, and treats a missing/null/`"unknown"`/unparseable `remote_type` as nothing to restore. Every restored row is re-judged through the same `LocationCriteria.meets` path, or it would come back mis-bucketed. Idempotent. |
| `--backfill-fit-versions` | Fill `JobFitScore.assessmentPromptVersion` from each score's stored analysis (TASK-711). The rubric version was only ever a key inside `fitScoreJSON`, so nothing could select on it; this mirrors it into a real column. **Zero LLM calls.** A row whose analysis records no version keeps a nil column — unknown, not v1. Idempotent. |
| `--fit-version-histogram` | Report how many stored scores sit on each rubric version, with each version's mean. Read-only. Reads the column, so run `--backfill-fit-versions` first. Scores from different rubrics are different measurements — v1 averaged 68.9 and v3 50.0 on the live store, so a threshold filter spanning both compares unlike things. |
| `--rescore-stale-fit-scores [--limit <n>] [--yes]` | Re-run the LLM over every succeeded score not on the current rubric. **The only mode that spends money** — about $0.0117 per score measured (8,858 prompt / 1,351 completion tokens on `gemini-3.7-flash`), so a full v1 sweep of 977 scores is ≈ $11.44. Prints the work set and the estimated cost and stops unless `--yes` is given; `--limit` tries a few first. Resumable: each score is committed as it lands and stamps the current version, so an interrupted run resumes exactly where it stopped and nothing already paid for is redone. Provider and model come from the store's settings; the **API key comes from `JOBHUNT_API_KEY`**, because the app's Keychain item is bound to the app's signing identity. |
...
| `--backfill-board-locations --input <path>` | Refill empty `Job.location` values from the board-location snapshot the ledger no longer holds (TASK-693). The discovery ledger keeps `rawJSON` for only one sweep per source and nils it on the next, so the boards' own location strings for already-ingested jobs are gone from the store; `--input` names the JSON snapshot taken before they vanished (`~/Documents/jobhunt-backups/keep-permanently/board-locations-<date>.json` — the only surviving copy, read-only). Matches each record on `DiscoveredPosting.dedupKey` first and a normalised URL second, counting the two separately. **Fills `location` and nothing else**: it never sets or infers `remoteType` (deriving an arrangement from location text is what erased 223 of them in TASK-708) and never overwrites a location that already says something, so a hand-edited value outranks the board. Does not re-judge `meetsCriteria` — run `--recompute-criteria` afterwards. **Zero LLM calls.** Idempotent. |
