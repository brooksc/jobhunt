...
func verify(src: DBHandle, context: ModelContext) -> VerifyResult {
    ...
    func label(_ s: String) -> String {
        s.padding(toLength: pad, withPad: " ", startingAt: 0)
    }
    ...
    let knownOrphanCaptures: Set = ["cap_b69c45c00be44806915ebf3674c138e2"]
    ...
    let sqlJobRows = queryRows(
        src,
        "SELECT id, job_number, status, company, title, fit_score, fit_status, extraction_status FROM jobs ORDER BY job_number"
    )
    ...
    for row in sqlJobRows {
        ...
        if !expectedStatuses.contains(sdJob.status.rawValue) {
            ...
            statusDetails
                .append(
                    "#\(num): sqlite=\(sqlStatus) sd=\(sdJob.status.rawValue) (expected one of: \(expectedStatuses.sorted().joined(separator: "|")))"
                )
        }
    ...
    if statusMismatches == 0 {
        ...
        for d in statusDetails.prefix(10) {
            print("       \(d)")
        }
    }
    ...
    if fitMismatches == 0 {
        ...
        for d in fitDetails.prefix(10) {
            print("       \(d)")
        }
    }

    let sqlExtractSucceeded = sqlCount(src, "jobs", "extraction_status = 'succeeded'")
    let sdExtractSucceeded = sdJobsAll.count(where: { $0.extractionStatus == .succeeded })
    if sqlExtractSucceeded == sdExtractSucceeded {
        r.ok(label("extraction succeeded count"), "\(sdExtractSucceeded)")
    } else {
        r.fail(
            label("extraction succeeded mismatch"),
            "SQLite \(sqlExtractSucceeded) vs SwiftData \(sdExtractSucceeded)"
        )
    }
    ...
    let sqlFSSucceeded = tableExists(src, "job_fit_scores") ? sqlCount(
        src,
        "job_fit_scores",
        "fit_status = 'succeeded'"
    ) : 0
    let sdFS = sdCount(context, JobFitScore.self)
    let sdFSSucceeded = (try? context.fetch(FetchDescriptor<JobFitScore>()))?
        .count(where: { $0.fitStatus == .succeeded }) ?? 0
    ...
    let sdJobsWithScore = sdJobsAll.count(where: { $0.fitStatus == .succeeded })
    let sdJobsWithRecord = sdJobsAll.count(where: { $0.fitStatus == .succeeded && !$0.fitScores.isEmpty })
    ...
    let sqlSpotRows = queryRows(
        src,
        "SELECT job_number, company, title, status, fit_score FROM jobs WHERE job_number IN (1,50,100,126,136)"
    )
