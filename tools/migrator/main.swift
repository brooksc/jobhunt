...
func parseArgs() -> Args? {
    let defaultInput = FileManager.default.homeDirectoryForCurrentUser
    ...
    guard let out = output else {
        ...
        fputs("Usage: JobhuntMigrator [--input <path>] --output <path>\n", stderr)
        return nil
    }
    return Args(inputPath: input, outputPath: out)
}
    ...
    if tableExists(src, "jobs") {
        for row in queryRows(src, "SELECT * FROM jobs") {
            ...
            let j = Job(
                ...
                status: row.str("status").flatMap { JobStatus(rawValue: $0) } ?? .saved,
...
guard FileManager.default.fileExists(atPath: args.inputPath) else {
    fputs("Error: input database not found at '\(args.inputPath)'\n", stderr)
    exit(1)
}

guard let srcDB = openReadOnly(args.inputPath) else { exit(1) }
...
print("")
print("Store written to: \(args.outputPath)")
